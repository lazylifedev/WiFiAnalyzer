...
@Composable fun BackupScreen(workspaceState:WorkspaceUiState,onBack:()->Unit,onOpenWorkspace:(Long)->Unit,onOperationSuccess:()->Unit={},access: FeatureAccessPolicy = FeatureAccessPolicy.from(com.lazyapps.wifianalyzer.billing.ProEntitlementState.Free),onOpenPro:()->Unit={},vm:BackupViewModel=viewModel()){
    LaunchedEffect(access.isPro) { vm.setAccess(access) }
    ...
    Scaffold(topBar={TopAppBar(title={Text(stringResource(R.string.backup_and_restore))},navigationIcon={IconButton(onClick=onBack){Icon(Icons.AutoMirrored.Rounded.ArrowBack,stringResource(R.string.back))}})}){padding->
        Column(Modifier.fillMaxSize().padding(padding).verticalScroll(rememberScrollState()).padding(AppSpacing.large),verticalArrangement=Arrangement.spacedBy(AppSpacing.medium)){
            Text(stringResource(R.string.backup_contents_description),style=MaterialTheme.typography.bodySmall);Text(stringResource(R.string.backup_excluded_settings),style=MaterialTheme.typography.bodySmall);Text(stringResource(R.string.backup_security_warning),color=MaterialTheme.colorScheme.error,style=MaterialTheme.typography.bodySmall)
            ...
            Card(Modifier.fillMaxWidth()){Column(Modifier.padding(AppSpacing.large),verticalArrangement=Arrangement.spacedBy(AppSpacing.small)){Text(stringResource(R.string.all_data),style=MaterialTheme.typography.titleMedium);Text(stringResource(R.string.workspace_count,workspaceState.workspaces.size));Button(onClick={if(access.canBackup){workspaceExport=false;create.launch(fileName("Backup"))}else restriction=AccessRestriction.BackupRequiresPro},modifier=Modifier.testTag("backup_all"),enabled=!state.busy){Text(stringResource(R.string.backup_all_action))}}}
            Card(Modifier.fillMaxWidth()){Column(Modifier.padding(AppSpacing.large),verticalArrangement=Arrangement.spacedBy(AppSpacing.small)){Text(stringResource(R.string.selected_workspace),style=MaterialTheme.typography.titleMedium);Text(workspaceState.selected?.name?:stringResource(R.string.none));OutlinedButton(onClick={if(access.canBackup){workspaceExport=true;create.launch(fileName(workspaceState.selected?.name?:"Workspace"))}else restriction=AccessRestriction.BackupRequiresPro},modifier=Modifier.testTag("backup_workspace"),enabled=!state.busy&&workspaceState.selected!=null){Text(stringResource(R.string.backup_workspace_action))}}}
            OutlinedButton(onClick={if(access.canRestore) open.launch(arrayOf("application/zip","application/octet-stream")) else restriction=AccessRestriction.RestoreRequiresPro},modifier=Modifier.testTag("restore_open"),enabled=!state.busy){Text(stringResource(R.string.restore_from_backup))}
